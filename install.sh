...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$tag_version" ]]; then
            ...
            tag_version=$(curl -4 -Ls "https://api.github.com/repos/agassiz/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        wget --inet4-only -N -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/agassiz/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    wget --inet4-only -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/agassiz/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        wget --inet4-only -O /etc/init.d/x-ui https://raw.githubusercontent.com/agassiz/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
