...
本指南介绍如何使用优化的Docker开发环境进行3x-ui开发，支持**真正的热重载**和**快速重建**，大幅提升开发效率。
...
### HTML/CSS/JS文件 - 真正的热重载
- ⚡ **0秒延迟** - 修改后立即生效
- 🔄 **无需重启** - 保存文件后直接刷新浏览器
- 📁 **支持所有前端文件** - HTML、CSS、JavaScript
- 🎯 **实时同步** - Docker挂载 + Debug模式
...
| 文件类型 | 优化前 | 优化后 | 提升 |
|---------|--------|--------|------|
| HTML/CSS/JS | 5-7分钟重建 | **0秒热重载** | ∞ |
| Go源代码 | 7-10分钟重建 | **5-7秒重建** | 85%+ |
| 首次构建 | 7-10分钟 | ~8分钟 | 20%+ |

### 智能缓存策略
1. **Go依赖缓存**: `go.mod`/`go.sum` 不变时复用
2. **外部资源缓存**: Xray和geo数据文件缓存
3. **代码层分离**: 只有代码变更时才重新编译
4. **Debug模式**: 前端文件从文件系统实时读取
...
./docker-dev.sh --force-rebuild     # 强制重新构建（不使用缓存）
```
...
### 开发环境文件
- `Dockerfile.dev`: 开发专用Dockerfile，优化了缓存层级
- `docker-compose.dev.yml`: 开发环境配置（Debug模式+挂载）
